...
// Si un Administrador entra aquí por error, lo regresamos a su panel
...
<head>
    ...
    <style>
        /* ================================================
           ESTILOS ESPECÍFICOS DEL PANEL DE MECÁNICO
           ================================================ */

        /* Espacio inferior para que el contenido no quede
           tapado por la barra de navegación en móvil */
...
<body class="bg-slate-50">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <!-- Sidebar oculto (la navegación está en la barra inferior) -->
        <aside class="w-64 bg-white border-r border-slate-200 flex flex-col hidden">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                    ...
                    <div class="relative w-full md:w-80">
                        <input
                            ...
                            placeholder="Buscar T-201, Motor, etc..."
                            class="w-full pl-11 pr-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-400 focus:outline-none text-sm shadow-sm transition-all bg-white">
                        <i class="fas fa-search absolute left-4 top-3.5 text-slate-400 text-lg"></i>
                    </div>
                </div>

                <div id="contenedor-historial" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6"></div>
            </div>
                ...
                <div class="bg-slate-100 rounded-2xl w-full max-w-2xl shadow-2xl overflow-y-auto max-h-[90vh] border border-slate-300">
                    ...
                    <div class="bg-slate-800 p-6 flex justify-between items-start text-white relative overflow-hidden">
                        <div class="relative z-10">
                            <span class="bg-indigo-500 text-white text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider shadow-sm">
                                Cierre de Orden
                            </span>
                            <h3 class="text-3xl font-black mt-2 tracking-tight" id="modal-unidad-txt">
                                UNIDAD: T-XXX
                            </h3>
                        </div>
                        <button onclick="cerrarModalFinalizar()" class="relative z-10 text-slate-400 hover:text-white text-3xl transition-colors">
                            &times;
                        </button>
                    ...
                    <form id="form-finalizar" onsubmit="guardarOrdenFinal(event)" enctype="multipart/form-data" class="p-6">
                        ...
                        <div class="flex gap-4 -mt-12 relative z-20 mb-6">
                            <div class="flex-1 bg-slate-900 rounded-xl p-4 shadow-lg border border-slate-700 flex flex-col items-center justify-center">
                                <span class="text-slate-400 text-xs font-bold mb-1 uppercase tracking-widest">
                                    <i class="far fa-play-circle mr-1"></i> Inicio
                                </span>
                                <input
                                    type="time"
                                    id="fin-hora-inicio"
                                    name="hora_inicio"
                                    class="bg-transparent text-emerald-400 font-mono text-xl text-center w-full outline-none pointer-events-none"
                                    readonly>
                            </div>
                            <div class="flex-1 bg-slate-900 rounded-xl p-4 shadow-lg border border-slate-700 flex flex-col items-center justify-center">
                                <span class="text-slate-400 text-xs font-bold mb-1 uppercase tracking-widest">
                                    <i class="far fa-stop-circle mr-1"></i> Fin
                                </span>
                                <input
                                    type="time"
                                    id="fin-hora-fin"
                                    name="hora_fin"
                                    class="bg-transparent text-red-400 font-mono text-xl text-center w-full outline-none pointer-events-none"
                                    readonly>
                            </div>
                        </div>

                        <!-- Clasificación y próxima cita -->
                        <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 mb-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <select
                                    id="fin-tipo"
                                    name="tipo_servicio"
                                    class="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-slate-700 font-medium outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all cursor-pointer"
                                    required>
                        ...
                        <div class="bg-white p-5 rounded-xl shadow-sm border border-slate-200 mb-6">
                            <label class="block text-xs font-bold text-slate-500 uppercase mb-2">
                                Reporte Técnico / Refacciones Usadas
                            </label>
                            <textarea
                                id="fin-descripcion"
                                name="descripcion_cierre"
                                rows="3"
                                class="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-slate-700 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 resize-none transition-all"
                                placeholder="Ej. Cambio de banda de distribución, 4 balatas y relleno de fluidos..."
                                required>
                            </textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            ...
                            <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col">
                                ...
                                <div class="flex-1 border-2 border-dashed border-slate-300 rounded-lg flex flex-col items-center justify-center bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer relative py-5 group">
                                    ...
                                    <p class="text-sm font-bold text-slate-600 px-2 text-center" id="nombre-archivo">
                                        Capturar o Subir
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col">
                                <div class="flex justify-between items-center mb-2">
                                    ...
                                    <button
                                        ...
                                        <i class="fas fa-undo-alt mr-1"></i> Borrar
                                    </button>
                                </div>
                        ...
                        <div class="flex gap-4">
                            <button
                                ...
                                class="w-1/3 bg-transparent border-2 border-slate-300 text-slate-600 font-bold py-3.5 rounded-xl hover:bg-slate-200 transition-colors uppercase tracking-wide text-sm">
                                Cancelar
                            </button>
                            <button
                                type="submit"
                                class="w-2/3 bg-slate-800 text-white font-bold py-3.5 rounded-xl hover:bg-slate-900 shadow-xl shadow-slate-300/50 transition-all transform hover:-translate-y-1 uppercase tracking-wide text-sm flex items-center justify-center gap-2">
            ...
            <div id="modal-escaner" class="fixed inset-0 bg-slate-900/90 backdrop-blur-sm hidden items-center justify-center z-[60] p-4 transition-opacity">
                <div class="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border border-slate-300 relative">
                    ...
                    <div class="bg-indigo-600 p-5 flex justify-between items-center text-white">
                        ...
                        <button onclick="cerrarEscanerQR()" class="text-indigo-200 hover:text-white text-3xl transition-colors leading-none">
                            &times;
                        </button>
                    </div>

                    <div class="p-6">
                        <p class="text-center text-slate-500 text-sm mb-4 font-medium">
                            Apunta la cámara al código QR del camión
                        </p>
                    ...
                    <div class="p-4 bg-slate-50 border-t border-slate-100 text-center">
                        <button onclick="cerrarEscanerQR()" class="px-6 py-2 bg-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-300 transition-colors text-sm">
                            Cancelar
                        </button>
                    </div>
                </div>
    ...
    " class="md:hidden">
        ...
        <!-- Escanear QR (botón circular en el centro) -->
        <button
    ...
    <script>
        /* ------------------------------------------------
           ...
           Cuando cambias de pestaña, el botón activo
           se pone morado y los demás se ponen grises
           ------------------------------------------------ */

        // Guardamos referencia a la función original
        var cambiarPestanaOriginal = cambiarPestana;

        // La reemplazamos con una nueva que hace lo mismo
        // pero también actualiza los colores de la barra inferior
